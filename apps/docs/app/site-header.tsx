...
//   行き先（はじめに・見本・Storybook）は帯が狭いと右のメニューに畳み、GitHub だけ帯に残す
...
export function SiteHeader({ current }: { current: 'home' | 'examples' }) {
  return (
    <Navbar
      ...
      actions={
        <Link variant="button" color="primary" href={REPOSITORY} target="_blank">
          GitHub
        </Link>
      }
    >
      ...
      <NavbarLink href={STORYBOOK} target="_blank" rel="noreferrer">
        Storybook
      </NavbarLink>
    </Navbar>
  );
