...
- 読み取り専用（readOnly）の TextField が、通常の欄と同じ見た目です。読み取り専用の形は、軸を立てて一から比べます（[ADR-0164](./adr/0164-principles-reorganization.md) の D11）。前の原則にあった「読み取り専用は下線だけ」は、参照画像（プロフィール画面）のキーと値の組を、読み取り専用の入力欄と読み違えたものでした。「参照画像にあったのは、読み取り専用のフォームではなく、キーと値のペアを示すコンポーネントです。」（2026-09-19）。「4 あとでにします。」（押せない状態の一覧の 7）。Textarea も同じです（[ADR-0124](./adr/0124-textarea.md)）
...
- 全体が押せる Card と LinkCard を、浮いた押すものに直します（2026-09-19 に決めました。原則 1・3）。薄い影を付け、hover で影を減らし、hover の輪郭の変化は外します。「カードは常に押せるとは限らず、ボタンしか押せない可能性があるため、常に浮くのではなく、カード全体が押せる場合のみ影を追加するかたちにします。」。hover の面の塗りを残すかは、軸を立てて比べます（[ADR-0167](./adr/0167-pressable-card-floats.md)）。決まったら [ADR-0129](./adr/0129-card-press.md) を置き換えます
