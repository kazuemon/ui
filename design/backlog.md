...
2026-09-18 に、原則と Select の決定から作りました。作る前に決めたこと（答えの原文）: Drawer の向きは「下を既定、左右も選べる」、Select のシートとは「見た目の部品を共有」（見出し・続きの印・トークン。引く操作は Select は自前、Drawer は Base UI）、指で操作していて画面が狭いときは Dialog・Popover を「両方シートにする」、指の Tooltip は「長押しで出す」。ADR はまだ書いていません。

- 共有にしたトークン: 影（`--shadow-overlay`・`--shadow-sheet`）と後ろの暗さ（`--color-backdrop`）を役割に上げ、開閉の動き（`--popup-*`）とシートの値（`--sheet-*`）を部品の層で共有にしました。Select の見た目は変わっていません（見た目の回帰テストで確かめた）
- 原則にない判断（仮の値で作った。比べて決める）。2026-09-18 に「残りも比較お願いします！」を受けて、軸 82（Dialog の角と大きさ）・83（下の操作の並べ方）・84（Tooltip の面）・85（Popover の × と矢印）・86（横の Drawer の幅と影）・87（開いた直後のフォーカス）のストーリーを作った:
  - Dialog の角をカードの角にした（部品を包むため）。Popover・Tooltip は浮かぶ面なので部品の角
  - Dialog・Drawer・Popover の題と説明の大きさは、軸 81 で B（題は見出しの 3 段目、説明は小さい本文）に決まった（「B でお願いします」。ADR はまだ）。「パソコンの場合、それぞれが内包するコンテンツに対して文字サイズが小さすぎます（確かめる（中身なし）など）」。題と説明は部品の props のまま持つ（読み上げの名前と説明を部品がつなぐため）
  - Dialog の幅（480px）・余白（24px）・下のボタンの右寄せ
  - Tooltip の面を、ほかの浮かぶ面と同じ白・細い輪郭にした。濃紺の塗りに白い文字の形とは比べていない。文字はキャプションの大きさ
  - Tooltip の既定は、下に出し、400ms で出す。影はほかの浮かぶ面より小さく淡い値（「ブラーがちょっと濃い目」「デフォルトの向きは下にしたい」「出てくるまでの時間はもう少し短めにしたい」を受けた仮の値）
  - Popover に閉じる × を置かない（シートでは置く）。矢印（本体を指す三角）も付けていない
  - シートの下のボタンを右寄せにした（幅いっぱいに縦に積む形とは比べていない）
  - Drawer の横のパネルの幅（360px）と影（浮かぶ面と同じ下向きの影）
  - 開いた直後のフォーカスは Base UI の既定（最初に Tab で止まるもの）。Dialog では右上の × になる
- 軸 82〜87 の答え（2026-09-18。ADR はまだ。記録を書くときに、比べるためだけのトークン（--dialog-radius・--_-actions-_・--color-tooltip*・--popover-*-display・--popover-close-space）を部品に畳み、Popover の × の形と initialFocus の props を消す）:
  - 82「82 は現行版」
  - 83「83 はデフォルト右寄せ、A と B が選べると嬉しいですが、このボタンはどのレイヤーで指定するものでしょうか？色の指定やアクションの変更（保存を挟むなど）をする場合に、ユーザー側で作るレイアウトなのか、コンポーネント側で責務を持つものなのかによるかなと思いました。」→ ボタンの文言・色・数・順は使う側（actions）、並べ方は部品（actionsLayout。Dialog は中央に浮かべるときはいつも右寄せ）と答えた
  - 84「84 は現行版がデフォルトで、影無しを選べる、がよさそうです。」→ shadow
  - 85「85 は現行版がデフォルトで、Aを選べる、がよさそうです。」→ arrow
  - 86「86 はAで良さそうです。」
  - 87「87 は現行がデフォルトで、autofocus 属性みたいなのでコントロールできますか？アクセシビリティ的にどれがいいのかは迷います。」→ 中身の要素の autoFocus を部品が拾う形にした。Base UI の既定は Dialog が最初に Tab で止まるもの（指で開いたときは面）、Drawer が面
- Drawer の半分の段: Base UI の snap points で、面を下にずらし、ずらした分だけ下に余白を足しています。Select のシートと違い、つまみを押しても半分と高さいっぱいを切り替えません。はじいたとみなす速さも Base UI の値です
- シートの区切り線: 上は中身がスクロールできるときだけ、下は操作を置いたときに下の影が出ているあいだだけ出します（「内部が scrollable でない場合は見出しと内容の divider を非表示に」「影がある場合は下にも divider が欲しい」）。Select のシートは変えていません
- はじいて閉じる動き: Base UI の例のようにはじいた強さで短くすると一瞬で消えるので、Select と同じくいつもシートの長さで滑らせます。半分の段から閉じるときは transition が始まらないので、キーフレームで滑らせています（Base UI を上げたら要らなくなるかを見る）
- Tooltip の長押し（500ms）は実機で確かめていません。iOS の Safari で文字の選択やメニューが出ないか、スクロールの始まりと取り違えないかを見ます
- 構造を固定する Provider（原則11「使う側が固定できる」）は、まだ部品ごとの `presentation` だけです
- AlertDialog（取り消せない操作の確かめで、外を押しても Esc でも閉じない形）は作っていません。いまは `dismissible={false}` で外を押しても閉じないようにできるだけです
