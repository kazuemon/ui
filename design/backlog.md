...
- **列を並べる `Grid` がありません（2026-09-22。README の「ページの枠」に足しました）。** `Stack` は縦か横の 1 列なので、「スマホ 1 列・PC 3 列」のような格子は書けません。見本の 15 ページのうち 6 ページで、`grid` のクラスを直に書きました（Tailwind を書かない縛りを守れない、最大の理由です）。決めること: 列の数（固定・画面の幅に応じて）、間隔（Stack の `gap` と同じ段）、子を同じ高さにそろえるか、入れ物の幅で列を減らす（`auto-fit`）か
