...
このファイルは「決めること」と「確かめること・既知の制約」の 2 部に分かれています。前半の「決めること」は、まだ決めていない判断の一覧です。決まったら ADR を書き、この一覧から消します。後半の「確かめること・既知の制約」は、実機で確かめることや Base UI 待ちなど、判断ではなく作業のメモで、確かめ終わったら消します。
...
- 選ぶ部品（Select・Checkbox・Switch・RadioGroup。Base UI には readOnly がある）の読み取り専用の形は決めていません。文字の欄の形（[ADR-0170](./adr/0170-readonly-field.md)。塗りなし・破線の輪郭・一段淡い文字）にそろえるかを、作るときに比べます。NumberField・PinField・MaskField・SearchField・DateField・TimeField は、文字の欄の形をそのまま使っています（[ADR-0187](./adr/0187-typed-field-series.md)）
...
- 文字のボタン（「コピー」など）に、グレー地を付けるかは決めていません
- 読み取り専用でグレー地が消えたとき、ボタン（消去など）をどう見せるかは決めていません
...
- 全角を半角に直したことを知らせる仕組み（`halfWidthNotice`）は MaskField だけにあります。NumberField など、ほかの欄にもそろえるかは決めていません
