...
- Select の使い方のストーリー「シートをはじく」のテストが、全体を流したときに 2 度落ちました（どちらも流し直すと通る）。落ちるのは、はじいたあとに本体の `aria-expanded` が `false` になるのを確かめるところで、閉じる動きが終わる前に見ています。待ち方（動きの終わりを待つ）を見直します
...
- 1つだけ置く箱は、Form の送信中（blocking）でも見た目を変えません。止める見た目は入力欄の部品にだけあります。チェックボックスとラジオも止めるかは決めていません
...
- 濃紺・青などの色の面の上に部品を置く仕組みがありません。いまは、青いカードの上で青いフォーカスの線が地との比 1.00 で見えず、濃紺の上でも 3.05 です。「青のカードにおいても filled な notice と同じように白いフォーカスリングになるようにしたいですね。これは props  で切り替えできるようなものですか？」「ユーザー側の任意コンポーネント内で利用するとしたときは props しかないかなと思っているのですが、2つの経路を作るのはいいと思いますか？」「kazuemon/ui で提供されるコンポーネント同士での親和性も維持できそうですね」
  ...
  - 面の上で部品ごとに決めること: 面と同じ色のボタン・ON のトグル・選んだ箱が溶ける（白いボタンを使う決まりにするか、面で差し替えるか）、キャプションなどの薄い文字と境界線の色、入力欄の枠線（軸 41 の L・M なら濃紺）、エラーの欄の外側の線（面では白になり「赤・白・白」）、「すべて選ぶ」の枠の文字の下の色（`--color-choice-frame-notch-bg`）
  - 軸 41 が決まったあとに、濃紺・青・ピンクの面の上で比べる
