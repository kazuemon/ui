...
- 終わりの分からない進み具合の動き（軸 167）を比べています。既定は現行版（流れる区切り）のつもりで、斜めの縞の案（D・E）を足しました
- 記事の読了のバー（軸 168）は、太さ 4px（`size="sm"`）・地ありを既定にし、2px（`size="xs"`。Progress だけ）と地なし（`track={false}`。Progress だけ）を選べるようにしました。読了のバーは部品にせず、Progress を Affix で上端に留める使い方として Docs に置いています
- 読了のバーの角（画面の端に接する端を丸めるか）はまだ答えをもらっていません。いまは丸いまま（`--bar-radius`）で、角だけを差し替えられるようにしてあります
