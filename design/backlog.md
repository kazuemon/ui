...
- 読み取り専用（readOnly）の TextField が、通常の欄と同じ見た目です。読み取り専用の形は決め直します（2026-09-19）。前は「下線だけ」でしたが、下線は文字のリンク、今日の日、選んだタブの印にも使うようになり、「下線が出てきたら読むだけ」が成り立たなくなりました。原則 8 には「読み取り専用の欄には塗りを持たせない」だけを残しています。軸を立てて比べます（[ADR-0164](./adr/0164-principles-reorganization.md) の D11）。「4 あとでにします。」（押せない状態の一覧の 7）。Textarea も同じです（[ADR-0124](./adr/0124-textarea.md)）
