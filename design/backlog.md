...
- **入れ子の型（`variant="nested"`）の余白が、上下左右でそろっていません（2026-09-22、見本のブログで気づきました）。** 画像から文までが上で 8px、文の下と左が 16px です。文の余白を「カードの余白（16px）− 画像の周りの余白（8px）」にして、下と左には外側の 8px が足され、上だけは画像が挟まるので足されないためです（[ADR-0014](./adr/0014-nested-card-inset.md) は余白の大きさだけを決めました）。案は 2 つです
  - 入れ子の型で、画像のすぐ後ろの文の上にだけ余白を足して 16px にそろえます（標準の型と文の位置がそろうので、こちらを推します）
  - 下と左も 8px に詰めます（標準の型と、文の位置が変わります）
- **横並びのカードの作りがありません（2026-09-22、見本のブログで気づきました）。** 画像を左、文を右に置くと、`CardImage` が 16:9 のままなので、画像が文より低くなり、下に白い帯が出ます。見本では `frameProps` の `className` で `sm:aspect-auto! sm:h-full` を当てて避けています（`!important` が要るのは、比率がインラインの style で入るため）。案は 2 つです
  - `Card` に向き（横並びの型）を足し、画像を高さいっぱいに広げ、狭い画面では縦に積みます
  - `CardImage` に「比率を持たない」指定（`ratio="auto"` で高さを親に任せるなど）を足します
...
- **候補の `label` と `value` のうち、選ぶと入力欄に入るのがどちらかが、分かりにくいです（2026-09-22、見本の記事一覧で気づきました）。** Autocomplete の `value` は入力欄の文字なので、候補の `value`（記事の slug など）は入力欄には入りません。見本では入力欄に題を入れたかったので、候補の `value` にも題を入れて避けています。JSDoc に「選ぶと入力欄に入るのは `label`／`value` のどちらか」を書くか、候補の `value` を要らなくする（`label` だけで足りる）か、を決めます
...
- 利用者向けの CSS（`tailwind.css`・`styles.css`）は、Tailwind の既定の色と影を消していません（消すのは Storybook の `globals.css` だけ）。利用者にも役割の色だけを使わせるなら、利用者向けでも消します
...
- Tailwind なしの入れ方（[ADR-0313](./adr/0313-css-entries.md)）で残っていること
  - 部品のクラスは接頭辞のない Tailwind の名前なので、使う側の CSS に同じ名前のクラス（Bootstrap の `.p-2`・`.border`・`.rounded` など）があると、部品にも効きます。接頭辞を付ける案は採りませんでした
  - `styles.css` は `@layer kazuemon` の中にあるので、使う側の要素へのスタイル（`button { … }`・`a { … }`）も部品に勝ちます。README では、それらをレイヤーに入れるよう書きました
  - テーマ（`theme.css`）が `html` に置く `text-autospace` と文字のなめらかさ（antialiased）は、Tailwind なしのアプリでもページ全体に効きます
  - 部品の中だけのリセット（[ADR-0314](./adr/0314-scoped-reset.md)）の `.kz-ui *` は、ページのすべての要素で祖先をたどる選択子です。要素がとても多いページでの速さは測っていません
- Tailwind ありの入れ方では、部品は使う側のページのフォント（`body` などに置いたもの）を受け継ぎます。Tailwind なしでは、いちばん外の部品の要素に Mulish を置きます。どちらかに寄せるかは決めていません
- `tailwindcss` の peerDependencies は、確かめた `^4.3` にしています。4.1・4.2 で動くかは確かめていません
- 使う側の入れ方（Vite の Tailwind なし・Tailwind v4 あり、Next.js の Server Components）は、`pnpm pack` した tarball を手元のアプリに入れて確かめました（2026-09-24）。CI では確かめていません
...
2026-09-22、見本のページ（料金プラン・ダッシュボード）を組んで気づいたことです。

- **`List` の印にアイコンを使えません。** 印は `dash`・`dot` だけです。料金プランの「チェックの付いた特徴」は、`flex` とアイコンを手で組み、文が折り返しても 1 行目の中央にそろうよう、高さを 1 行分（`h-[1lh]`）にした入れ物を書いています。`ListItem` にアイコンの指定（`icon`）があれば、この組み立てが要りません。決めること: アイコンの色と大きさ（成功の緑・文と同じ色）、折り返したときの 2 行目の頭のそろえ方、読み上げ（印は飾りなので読まない）
- **`Table` を、狭い画面で横に送る形がありません。** スマホではカードや列からはみ出します。見本では、ダッシュボードの注文の表と料金プランの比べる表を、`overflow-x-auto` の div で包んでいます。決めること: `Table` に「はみ出したら横に送る」指定（内部は `ScrollArea`、端の影で続きを知らせる）を足すか、包む部品を別に出すか。最初の列（見出しの列）を固定するかも一緒に決めます
- **`Text` に、大きい文字の段がありません。** 料金の「¥980」のような大きな数字は、`Stat`（ラベルが必須）にも見出しにも合わず、見本ではトークンのクラス（`text-(length:--text-heading-1)`）を直に書いています。`Text` の `size` が `sm` と既定だけで、見出しの段（`heading-1`〜）は `Heading` のみです。決めること: `Text` に見出しの段の `size` を足すか、価格や数字向けの部品を別に作るか（`label`・`caption` の見た目は [ADR-0260](./adr/0260-text-variants.md) で現行どおりに決まりました。この項目とは別です）

### ドキュメントサイトの見本のページ

- **見本の画面が、Storybook（`src/samples/`）とドキュメントサイト（`apps/docs/examples/`）の 2 か所にあります（2026-09-20）。** ドキュメントサイトの `/examples/<名前>` に移すとき、`.stories.tsx` の中に直接書いてあった画面（ドキュメント・一覧・SNS・設定）をそのまま写しました。同じ画面を直すときは 2 か所を直すことになるので、どちらかに寄せます。寄せ方の案は、画面を `src/samples/*.tsx`（ストーリーではないファイル）に出し、ライブラリに見本用の入口（`@kazuemon/ui/samples` など）を足して、ストーリーと docs の両方がそれを読む形です。入口を公開するかどうか（配布物に入れるか）も一緒に決めます
- 写せなかったもの: 「Markdown の記事」の見本は、ライブラリの中だけの見た目（`figureImageClassName`・`inlineStyles`）を使っているため、ドキュメントサイトには置いていません。公開の形が決まったら足します
