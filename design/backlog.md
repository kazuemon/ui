...
2026-09-19 に作りました。決定は [ADR-0177](./adr/0177-pagination-current.md)・[ADR-0178](./adr/0178-pagination-shape.md) です。
...
2026-09-19 に作りました。今の見出しの印（軸 165）と入れ子の見せ方（軸 166）は比べている途中です。

- 行の高さは部品の高さ（44px）にせず、文字の上下に少し空けるだけにしています。文字はラベルと同じ大きさで、入力方式で変えません。指で押しやすいかは、狭い画面で Collapsible に入れたときに実機で確かめます
- 利用者が選ぶ色（`color`）は持っていません。軸 165 で青い印を選べるようにするなら、`currentIndicator` にするか `color` にするかを決めます
...
- 2026-09-19 に Progress を作りました。バーの形・色・太さ・3 層の並びは `src/internal/bar/` で Meter と共有し、トークンも `--bar-*` にまとめました。範囲による色は Meter だけのものです
- 終わりの分からない進み具合の動き（軸 167）と、記事の読了のバーの太さ・地・角（軸 168）を比べています。読了のバーは部品にせず、Progress を Affix で上端に留める使い方として Docs に置いています。軸 168 で現行版以外が選ばれたら、読了のバー向けの形を Progress で選べるようにします
