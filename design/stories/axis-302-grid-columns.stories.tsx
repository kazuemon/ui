...
export const Columns: Story = {
  ...
  render: () => (
    ...
    >
      ...
      <p>
        画面の幅で列の数を変えるときは、どちらの案でも <code>{'columns={{ base: 1, md: 3 }}'}</code>{' '}
        のように段ごとの数を渡します。この軸で比べるのは、画面の幅ではなく、置いた入れ物の幅で列を減らすかです（サイドバーの横のような、画面より狭い入れ物に置いたとき）。
      </p>
      <p>
        推奨は現行版です。<code>columns</code> の意味が Masonry
        と同じままで、書いたとおりの列の数になります。見本の「スマホは 1〜2 列、広い画面は 3〜4
        列」は段ごとの数で書けるので、入れ物の幅で減らしたいのは、狭い入れ物に置くときだけです。
      </p>
    </Comparison>
  ),
};
