...
- **後半: 2026-09-12 開始**
  ...
  - **次にやること**: 残りの軸は [`principles.md` の「後半の軸」](./principles.md#後半の軸storybook-で1軸ずつ詰める) にあります。軸 12 までが終わりました。次は「focus-visible（キーボード操作時のフォーカス）」をおすすめとして、ユーザーに選んでもらうところです。ボタン・リンク・トグルは、hover・押下・Disabled の見た目まで決まりましたが、キーボードで操作したときのフォーカスの見た目がまだないためです。原則2の入力欄のフォーカス（2px の青の枠線）を流用するかどうかも、ここで決めます。ほかの候補は、入力欄の prefix・suffix を部品として作る、和文の縦位置の補正、loading、Warning、Select の選択肢の見た目とボトムシート、指用の高さ 44px（実機）、リンクの大きさです
