...
2回に分けて比べました。比較は Storybook の `Design Review/13 キーボード操作時のフォーカス`（[`stories/axis-13-focus-visible.stories.tsx`](../stories/axis-13-focus-visible.stories.tsx)）です。「通常」（Tab キーで動かして確かめる）と「フォーカス中」（全部品のフォーカスを固定）の2列に並べました。
