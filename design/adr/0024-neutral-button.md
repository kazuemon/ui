...
- **追記（[ADR-0035](./0035-field-addon.md)）**: 決定2の「検索ボタンなど」を見直しました。suffix に入れるボタンは、パスワードの表示・非表示のように入力欄そのものを操作するものに限ります。検索のように入力した内容を送るボタンは、入力欄の外に独立したボタンとして置きます。ユーザーのメモは「suffix に入れるアクションボタンと言うとパスワードの表示非表示くらいしか浮かばないので、そもそも検索だったら外に出すかなと思いました。」でした。外に置くボタンは、グレーの入力欄と見分けがつくよう、グレーではなく色のボタンか枠線のボタンにします
