...
- ステータス: Accepted（ボタンの塗りは [ADR-0190](./0190-field-addon-kinds.md) で一部を置き換え）
- 日付: 2026-09-13
- ラウンド: 後半 軸17
...
- **追記（README の進捗から移した記録）**: 上の「分かっていること」のうち、suffix のボタンのフォーカスの見せ方と、文字の prefix の読み上げは、軸22（[ADR-0040](./0040-field-addon-rest.md)）で決まりました。
- **追記（[ADR-0190](./0190-field-addon-kinds.md)）**: 決定の表にある「ボタン: 平らな要素（原則3）」という記述を一部置き換えました。NumberField・PinField・SearchField などを作るにつれ、ボタン（`FieldAddonButton`）は、ふだんから文字の塊と同じグレー地を持つ形に決まりました（hover・押下は、その地の上に文字の色を重ねます）。塗りのないアイコンは「押せない意味の説明」（SearchField の虫眼鏡）として別に扱い、グレー地は「押せる」ことの印にそろえています。押下で沈む・キーボードのフォーカスの線（ADR-0040）などの決定は変わりません。
