...
比較は Storybook の `Design Review/26 リンクの残り`（[`stories/axis-26-link-rest.stories.tsx`](../stories/axis-26-link-rest.stories.tsx)）です。ストーリーは2つあります。比べたときは部品がどちらも持たなかったので、ストーリーの中で組み立てました。比較は1回です。そのあとのメモで、塗りのボタンのリンク、↗ の扱い、キーボード、押せないリンク、新しいタブで開くときの読み上げと ↗、文字のリンクの ↗ の大きさと下線が決まりました。
...
| ボタンの見た目のリンク（`render`）        | Button に `<a href>` や Next.js の Link を渡す。見た目は Button と同じで、要素だけが `<a>` になる。appearance・color はすべて使える（塗り・枠線、青・ピンク・赤・グレー・白）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
