...
比較は Storybook の `Design Review/41 フォーカスの色と部品の色`（[`stories/axis-41-focus-color.stories.tsx`](../stories/axis-41-focus-color.stories.tsx)）です。色なし・青・ピンクの Select（本体の枠線と、その下のエラーの Select）、部品を並べたフォームの画面、お知らせの中、濃紺・青のカードに置いた部品の6列に、現行版と13案（A〜M）を並べました。
