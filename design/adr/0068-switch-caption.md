...
比較は Storybook の `Design Review/48 トグルとキャプションの置き方`（[`stories/axis-48-switch-caption.stories.tsx`](../stories/axis-48-switch-caption.stories.tsx)）です。A〜E は共通のルールとして、トラックとラベルを部品の高さの1行（マウス用 40px・指用 44px・大きい指用 52px）の縦の中央にそろえ、キャプションはこの行の外に置きます。囲み（区切り線・囲み）があっても、トラックはこの1行の中です。
