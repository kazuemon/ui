...
- 分かったこと（実装で判明): `BaseField.Root` を使う部品（Field を通るかどうかによらず、Checkbox・Switch を含め既存のもの全部）を `<Form>` の中に置くと、`required` などブラウザのネイティブな制約を Base UI 自身が送信のたびに確かめ、正しくない欄があれば先にその欄へフォーカスを移し、アプリの `onSubmit`・`onFormSubmit` を呼ばずに止めることが分かりました。文もブラウザの既定（ローカライズされた `validationMessage`）になります。もとの実装（素の `<form noValidate>`）ではこの検証が効かず、「値を確かめるのはアプリ」（`onSubmit`・`validate`・`errors` の中で決める）が唯一の経路だったため、ブラウザネイティブには寄せたくないという判断のもと、この副作用を打ち消しました。`required` を渡しても、どの欄もブラウザの制約（`required`・隠れた input の `required` 属性）は付けず、`aria-required` だけを直に付けて必須であることを伝えます（見た目の印はそのまま）。valueMissing が起きないので Base UI 自身の検証は通り、送信を止め文を出すのは欄の `validate` と Form の `errors` だけになりました。`type`・`pattern`・`minLength` など、`inputProps`（TextField・Textarea など）で自分で渡した属性は今回の対象外で、ブラウザの制約のまま残ります。渡さない・`validate` で確かめる、のどちらかで避けてください。`PinField`（`OTPField.Root`）は内部で `length` から `minLength`・`maxLength` を常に付けており、Base UI にこれを外す設定がないため、桁の途中で送信すると引き続きブラウザの既定の文で止まります（`required` の有無によりません）。`src/stories/required-mark.stories.tsx`・`src/components/checkbox/Checkbox.stories.tsx` の該当するストーリーは、アプリが文を決める元の形に戻しました
