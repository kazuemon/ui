...
2回に分けて比べました。比較は Storybook の `Design Review/18 Select の選択肢`（決めた時点のコミット `a1aadd1`。`git checkout a1aadd1 && pnpm storybook` で開けます）です。部品の浮かぶ部分の見た目をトークン（`--select-popup-*`・`--color-select-*`）にして、行ごとに切り替えて比べました。
