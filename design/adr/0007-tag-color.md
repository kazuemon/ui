...
- **追記（[ADR-0028](./0028-default-color.md)）**: この ADR で決まったのは、淡い面に同じ色相の濃い文字を載せる**塗り方**です。水色という色ではありません（「タグは青色で決まっているわけではなく、色の乗せ方がいいなと思ったところでした。」）。タグの色は利用者が青・ピンク・グレーから選び、指定しないときはグレーです。水色のトークン（`--color-tag`、`--color-on-tag`）は、比較のストーリーの「採用」の印で使っているため残しています
