...
比較は Storybook の `Design Review/36 黄色の塗りのお知らせと操作の場所`（[`stories/axis-36-notice-warning-filled.stories.tsx`](../stories/axis-36-notice-warning-filled.stories.tsx)）です。1列目は塗りの4色、2列目は警告の3つの見た目（`soft`・`filled`・`outline`）、3列目と4列目は操作の場所に枠線のリンクを置いたとき（4列目は hover で固定）です。変えたのは次のトークンだけです（`src/components/Notice.tsx` が読む）。
