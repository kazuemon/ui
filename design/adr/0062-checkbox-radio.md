...
比較は Storybook の `Design Review/40 チェックボックスとラジオ`（決めた時点のコミット `5de918a`。`git checkout 5de918a && pnpm storybook` で開けます）です。変えたのは箱の大きさ・チェックボックスの角・印の太さ・ラジオの丸の比・横の文字との間・選んでいない箱の塗りだけで、行の高さはどの案もトグルと同じ部品の高さです。
