...
比較は Storybook の `Design Review/32 Select の選んだ項目の色`（[`stories/axis-32-select-selected-color.stories.tsx`](../stories/axis-32-select-selected-color.stories.tsx)）です。列は色なし（neutral）・青（primary）・ピンク（secondary）のそれぞれで「選んだ項目に hover」「別の項目に hover」、行は現行版と4案です。変えたのは次のトークンだけです（`src/components/Select.tsx` の `selectedTokens` が読む）。
