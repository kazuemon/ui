...
5回に分けて比べました。比較は Storybook の `Design Review/16 loading 状態`（[`stories/axis-16-loading.stories.tsx`](../stories/axis-16-loading.stories.tsx)）です。部品にはまだ送信中の状態がなかったので、比べるための仕組みをストーリーの中で組み立てました。
