...
比較は Storybook の `Design Review/38 フォーム全体を送っているあいだ`（[`stories/axis-38-form-submitting.stories.tsx`](../stories/axis-38-form-submitting.stories.tsx)）です。印は、どの案でも送信のボタン（`Button` の `loading`）だけで、欄には回る円を出しません。
