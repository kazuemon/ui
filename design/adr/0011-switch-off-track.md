...
- **追記（[ADR-0029](./0029-disabled-refine.md)）**: OFF のトラックの塗りを、入力欄のグレー（`--color-field`、`#F2F4F4`）から、グレーのボタンの塗り（`--color-switch-off` = `--color-neutral`、`#EFF0F1`）に変えました。白地との比は 1.14:1 で、3:1 の例外のままです。OFF のトラックに薄い枠（1px）を付ける案も比べましたが、採りませんでした。押せない OFF のトラックは、明るいグレー（`#E1E3E4`）です
- **追記（ADR-0067）**: OFF のトラックの塗りを、どこでも `#E1E3E4`（それまで押せないときだけの色）に変えました。チェックボックス・ラジオの選んでいない箱の色ともそろえています。3:1 の例外は変わりません
