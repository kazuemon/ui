...
// バー: 地（--bar-track。トグルの OFF・選んでいない箱と同じグレー）の上に、値までを部品の色で塗る。角は小物と同じ pill（--bar-radius）
...
export const barStyles = tv({
  slots: {
    ...
    track:
      'relative col-span-full h-(--bar-height) overflow-hidden rounded-(--bar-radius) bg-(color:--bar-track)',
    indicator: [
